(function(){
  ...
  function runSimulationFromSignals(signals, opts){
    ...
    for (const s of signals){
      ...
      let win;
      if (deterministic){
        // deterministik: long için fiyat tp'ye, short için tp'ye ulaşıyor gibi davran (öğretici mod)
        win = true;
      } else if (useConfidence && typeof s.confidence === 'number'){
    ...
    return { equity, capital, trades, winRate, totalRet, maxDD, avgPnl };
  }

  // Sayfa yüklenince verileri getir, PnL grafiğini çiz, filtreleri bağla
  async function load(){
    const r = await fetch('/api/signals');
    ALL_SIGNALS = await r.json();
    ...
    if (kpiSignals) kpiSignals.textContent = ALL_SIGNALS.length;
    ...
    function rowVisible(tr){
      ...
      const ts = parseISO(tr.dataset.ts);
      ...
      let ok = true;
      if (q) ok = ok && rSym.toLowerCase().includes(q);
      if (sym) ok = ok && rSym === sym;
      if (side) ok = ok && rSide === side;
      if (fromDate?.value){
        const fd = new Date(fromDate.value);
        if (ts) ok = ok && ts >= fd;
      }
      if (toDate?.value){
    ...
    [fromDate, toDate].forEach(el=>el?.addEventListener('change', applyFilters));
    segs.forEach(b=>{
    ...
    const rowChecks = document.querySelectorAll('.rowSel');
    selAll?.addEventListener('change', ()=>{
      rowChecks.forEach(c=>{ if (c.closest('tr').style.display !== 'none') c.checked = selAll.checked; });
    });
  }
  ...
  async function attachSimulation(){
    ...
    btn.addEventListener('click', ()=>{
      // Hangi sinyaller kullanılacak?
      const useSelected = document.getElementById('useSelectedRows').checked;
      const useFilters  = document.getElementById('useActiveFilters').checked;
      let signalsToUse = [];

      const rows = Array.from(document.querySelectorAll('#tbody tr'));
      if (useSelected){
        const sel = rows.filter(tr => tr.querySelector('.rowSel')?.checked);
        if (sel.length){
          const idxs = sel.map(tr => parseInt(tr.dataset.idx, 10));
          signalsToUse = idxs.map(i => ALL_SIGNALS[i]).filter(Boolean);
        }
      }
      if (!signalsToUse.length && useFilters){
        const visible = rows.filter(tr => tr.style.display !== 'none');
        const idxs = visible.map(tr => parseInt(tr.dataset.idx, 10));
        signalsToUse = idxs.map(i => ALL_SIGNALS[i]).filter(Boolean);
      }
      if (!signalsToUse.length){
        signalsToUse = ALL_SIGNALS.slice(); // hepsi
      }
      ...
      const deterministic   = document.getElementById('fixedWinLoss').checked;

      const res = runSimulationFromSignals(signalsToUse, { capital0, feePct, tpPct, slPct, useSignalLevels, useConfidence, deterministic });
      ...
      document.getElementById('simAvgPnl').textContent   = res.avgPnl.toFixed(2);
